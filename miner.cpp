...
namespace LLP
{
	...
	{
		Packet RESPONSE;
		//std::cout << "WaitWorkUpdate Start. " << std::endl;
		...
		for (;;)
		{
			...
			if (RESPONSE.IsNull()) {
				...
				//std::cout << "WaitWorkUpdate Response is Null. " << std::endl;
				return NULL;
			}
			if (RESPONSE.GetHeader() != BLOCK_DATA) {
				//std::cout << "WaitWorkUpdate Reset Packet. " << std::endl;
		...
		//std::cout << "WaitWorkUpdate: Response.GetData() = " << std::string(data.begin(), data.end()) << std::endl;

		pWork->m_pBLOCK = DeserializeBlock(std::vector<unsigned char>(data.begin() + 4, data.end()));
		//std::cout << "WaitWorkUpdate got a Response.  Block Height: " << pWork->m_pBLOCK->GetHeight() << std::endl;
		//pWork->m_pBLOCK->printBlock();

		//First four bytes of the packet are the share difficulty for the pool?
		pWork->m_unBits = bytes2uint(RESPONSE.GetData());	//pool variant
		//pWork->m_unBits = pWork->m_pBLOCK->GetBits();		//solo variant
		//std::cout << "Pool Bits: " << std::hex << pWork->m_unBits << " -> Difficulty: " << std::dec << Core::GetDifficulty(pWork->m_unBits) << std::endl;
