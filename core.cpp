...
namespace Core
{
	ServerConnection::ServerConnection(std::string ip, std::string port, std::string login, std::vector<std::string> comPorts, int nMaxTimeout, bool benchmark)
		: IP(ip), PORT(port), TIMER(), m_szLogin(login), nThreads(comPorts.size()), nTimeout(nMaxTimeout), bBenchmark(benchmark), THREAD(boost::bind(&ServerConnection::ServerThread, this))
	{
		...
		//launch new threads for each com port
		for (int nIndex = 0; nIndex < nThreads; nIndex++)
		{
			MinerThread * minerThread = new MinerThread(nIndex, comPorts[nIndex], 115200);
	...
	void ServerConnection::ResetThreads()
	{
		...
		for (unsigned int nIndex = 0; nIndex < THREADS.size(); nIndex++)
		{
			...
			//std::cout << "Reset Threads" << std::endl;
		}
	...
	void ServerConnection::ServerThread()
	{
		...
		while (true)
		{
			...
			if (nPingCount >= 8)
			{
				...
				for (int nIndex = 0; nIndex < nThreads; nIndex++)
				{
					THREADS[nIndex]->SetIsShutdown(true);
			...
			try
			{
...
#else
					("\r[Stats] %.03f MH/s | Shrs/24h %.03f | A=%u R=%u | Diff=%.03f/%.3f | %02d:%02d:%02d:%02d\n",
						MegaHashesPerSecond, (double)nBlocksFoundCounter / (double)elapsedFromStart * 86400.0, nBlocksAccepted, nBlocksRejected, diff, dDiff, t.tm_yday, t.tm_hour, t.tm_min, t.tm_sec);
#endif
				...
				for (unsigned int nIndex = 0; nIndex < THREADS.size(); nIndex++)
				{
					...
					if (THREADS[nIndex]->GetIsNewBlock())
					{
						...
						if ((pWork))
						{
							...
							if (nBestHeight == nHeight)
							{
								...
								//std::cout << "New Height: " << nBestHeight << " Current Height: " << nHeight << std::endl;
								//std::cout << "Asking for new block." << std::endl;
					...
					else if (THREADS[nIndex]->GetIsBlockFound())
					{
						...
						//Share Diff
						uint1024 TheTarget2 = THREADS[nIndex]->GetBlock()->GetHash();
						uint64_t *ptarget2 = (uint64_t*)&TheTarget2;
						unsigned long nHighBitsShare = ptarget2[15];
						double dDiff = 64.0 - log2(nHighBitsShare);

						//network diff
						double networkDiff = Core::GetDifficulty(THREADS[nIndex]->GetBlock()->GetBits());
						double networkDiffLog = 34 + log2(networkDiff);
						//pool diff
						double poolDiff = Core::GetDifficulty(THREADS[nIndex]->GetBits());
						double poolDiffLog = 34 + log2(poolDiff);
						//===========================================================================
						...
						if (RESPONSE == 200)
						{
							...
							nBlocksAccepted++;
							THREADS[nIndex]->GetBlock()->SetNonce(nNonce + 1);
							THREADS[nIndex]->SetIsNewBlock(false);
							THREADS[nIndex]->SetIsBlockFound(false);
						}
