...
namespace Core
{
	...
	class MinerThread
		...
		private:
			...
			int m_nThroughput;
			//Com port name and baud rate
			std::string m_sComPort;
			int m_nBaudRate;

		public:			
		
			MinerThread(unsigned int Thread_Id, std::string comPort, int baudRate);
			...
			//Main Miner Thread. Bound to the class with boost.
			void SK1024Miner();
			...
			unsigned int					GetThreadID()          const   {   return this->m_ThreadID;               }
			const bool				        GetIsShutdown()		const	{ return this->m_bShutdown; }
			const std::string				GetComPort()		const	{ return this->m_sComPort; }
			const int						GetBaudRate()		const { return this->m_nBaudRate; }
...
//			void	SetGPUData(GPUData* data)					{	this->m_pGPUData = data;			}
			void    SetThreadID(unsigned int Thread_Id)         {   this->m_ThreadID = Thread_Id;          }
			void	SetIsShutdown(bool bShutdown)				{   this->m_bShutdown = bShutdown; }
			void	SetComPort(std::string sComPort)			{	this->m_sComPort = sComPort; }
			void	SetBaudRate(int nBaudRate)					{ this->m_nBaudRate = nBaudRate; }
